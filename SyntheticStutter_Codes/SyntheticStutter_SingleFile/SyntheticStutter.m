...
'Simulation Breakdown – Performs lagged correlation/causal analysis between model variables (x2, FreeEnergy, Precision, Uncertainty, Freeze) within a ±0.5s window. Produces a directed graph with edge weights.'
''
...
function customReset()
    ...
    % restart using the correct entry‐point in SyntheticStutter.m
    SyntheticStutter();
end
...
%SIMULATIONCALLBACK   Run sim, then post‐process into multiple, human‐like blocks.
    ...
    ud.modelParams.freezeThreshold = 5;
    ...
    ud.config.smoothingWindow    = 3;  % ~3 ms
    ud.constants.smoothingWindow = 3;
  ...
  'modelParams',params.modelParams ...
    ...
    %% 8) Log freeze duration breakdown
    nShort = sum(simResults.blockDurations < 0.150);
    nLong  = sum(simResults.blockDurations >= 0.150);
    logMessage('INFO', sprintf('→ Short freezes (<150 ms): %d', nShort));
    logMessage('INFO', sprintf('→ Long  freezes (≥150 ms): %d', nLong));
